...
export function createTelemetry(config) {
  ...
  const prefix = typeof config.metricPrefix === 'string' && config.metricPrefix.length > 0
    ? config.metricPrefix
    : 'adapter';
  ...
  const validationErrors = meter.createCounter(`${prefix}.validation.errors`, {
    description: 'Number of validation errors'
  });

  const states = new Map();
  const circuitState = meter.createObservableGauge(`${prefix}.circuit.state`, {
  ...
  function setCircuitState(session, state) {
    ...
    const value = Object.hasOwn(map, state) ? map[state] : 0;
