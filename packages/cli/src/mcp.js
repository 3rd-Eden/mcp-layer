 ...
 * @param {{ server?: string, config?: string, spinner: boolean, transport?: string, timeout?: number }} opts - CLI options for server selection and spinner display.
...
export async function catalog(opts) {
  ...
  try {
    ...
    gate.start();
    session = await connect(info.config, info.name, { transport: opts.transport, timeout: opts.timeout });
