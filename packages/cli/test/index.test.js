 ...
 * @param {{ cwd?: string, env?: Record<string, string> }} [options] - Spawn options for working directory.
...
/**
 * Validate strict guardrails are enabled by default.
 * @returns {void}
 */
function defaultsCase() {
  const config = defaults();
  assert.equal(config.guardrails.profile, 'strict');
}

/**
 * Run default configuration suite.
 * @returns {void}
 */
function defaultsSuite() {
  it('enables strict guardrails by default', defaultsCase);
}
...
/**
 * Build environment overrides for isolated stateful session tests.
...
/**
 * Validate session command execution with autogenerated session ids.
 * @returns {Promise<void>}
 */
async function sessionExecCase() {
  const setup = await setupconfig();
  ...
  try {
    ...
    assert.equal(Array.isArray(data.content), true);
    assert.equal(data.content[0].text, 'hello');
    assert.equal(result.stderr.includes('Session started:'), true);

    const list = await runcli(['session', 'list', '--format', 'json'], { env });
    const sessions = parsejson(list.stdout);
    assert.equal(Array.isArray(sessions), true);
    assert.equal(sessions.length >= 1, true);
  } finally {
    await runcli(['session', 'stop', '--all'], { env });
  }
}

/**
 * Validate named session reuse and targeted stop behavior.
 * @returns {Promise<void>}
 */
async function sessionNamedCase() {
  const setup = await setupconfig();
  const env = await sessionenv();
  ...
  try {
    ...
    const data = parsejson(second.stdout);
    assert.equal(data.content[0].text, 'two');
    assert.equal(second.stderr.includes('Session started:'), false);

    const stopped = await runcli(['session', 'stop', '--name', 'alpha', '--format', 'json'], { env });
    const payload = parsejson(stopped.stdout);
...
/**
 * Run stateful session command coverage.
 * @returns {void}
 */
function sessionSuite() {
  it('executes session operations and prints autogenerated ids', sessionExecCase);
  it('reuses named sessions and stops by name', sessionNamedCase);
}
 ...
 */
function cliSuite() {
  describe('defaults', defaultsSuite);
  ...
  describe('session commands', sessionSuite);
}
